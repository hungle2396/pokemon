...
import Personal_Pokemons from "./Personal_Pokemons";
...
const Pokedex = () => {
    ...
    const get_Pokemons = async (url) => {
        ...
        let pokemons_List = await Promise.all(response.data.results.map(async (pokemon_data) => {
            return await axios.get(pokemon_data.url);
        }));

        pokemons_List = pokemons_List.concat(Personal_Pokemons);
    ...
    const renderPokemons = pokemons.map((pokemon) => {
        ...
        if (pokemon.data.name.includes(term)) {
            ...
            let pokemon_img = "";
            let pokemon_background = "";

            if (pokemon.data.id > 11000) {
                pokemon_img = pokemon.data.sprites;
                pokemon_background = "rainbow"
            } else {
                pokemon_img = pokemon.data.sprites.other["official-artwork"].front_default;
                pokemon_background = pokemon.data.types[0].type.name;
            }

            return (
                <li key={pokemon.data.id} className="search__result--item">
                    <Link to={`/${pokemon.data.id}`} className={`search__result--link search__result--link--${pokemon_background}`}>
                        <div className="search__pokemon--id"># {pokemon.data.id}</div>
                        <figure className="search__pokemon--figure">
                            <img className="search__pokemon--img" src={pokemon_img} alt="pokemon image" />
                        </figure>
